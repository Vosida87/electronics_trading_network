...
Для создания API CRUD используем rest_framework и его viewsets.ModelViewSet и, конечно, создаём сериализатор (в нём поле задолженности указывается только для чтения и тем самым его нельзя поменять через API) 
***  
Для выполнения другой части задания используем django_filters.  

### 5 пункт - Права доступа.
Изначально в той модели от которой я наследовал пользователя есть is_active, поэтому с ним и работаем  
Используем permissions.BasePermission для проверки статуса этого поля  
Также написаны тесты для этого ограничения
***
### Coverage 99%
